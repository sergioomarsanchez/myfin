...
function TransactionCard({
  ...
  function handleDelete(id, transactionAccount) {
    Swal.fire({
      title: `Delete Transaction from date ${transaction.date.slice(
        2,
        10
      )}, Are you sure?`,
      ...
      if (result.isConfirmed) {
        try {
          ...
          await axios.put(
            `https://myfin-sergioomarsanchez.vercel.app/api/accounts/` +
              transactionAccount,
            { balance: newBalance }
          );
          dispatch(
            updateAccBalance({ id: transactionAccount, newBalance: newBalance })
          );
          await axios.delete(
            `https://myfin-sergioomarsanchez.vercel.app/api/transactions/` + id
          );
          Swal.fire({
            color: "white",
            background: "#141c24",
          ...
          dispatch(
            updateTotals({
              ...
              currency: entityName.toLowerCase().includes(" ars")
                ? "ars"
                : "usd",
  ...
  return (
    <div key={transId} className={style.container}>
        ...
        <tbody>
          <tr className={style.trTitle}>
            ...
            {transaction.type === "credit" ? (
              <th style={{ color: "#4ada84" }}>
                + ${parseFloat(transaction.amount).toFixed(2)}
              </th>
            ) : (
              <th>- ${parseFloat(transaction.amount).toFixed(2)}</th>
            )}
          </tr>
        </tbody>
      </table>
      ...
      <div
        ...
        onClick={() => handleDelete(transaction._id, transaction.account)}
      >
        X
      </div>
    </div>
