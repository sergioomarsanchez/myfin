...
function MonthlyGraphic({ year, month, type, yearTransactions }) {
  ...
  useEffect(() => {
    ...
    setTotalCredit(
      graphicData[Number(month) + 1]["credit"].reduce(
        (total, transactions) => total + transactions
      )
    );
    setTotalDebit(
      graphicData[Number(month) + 1]["debit"].reduce(
        (total, transactions) => total + transactions
      )
    );
  }, [graphicData]);
    ...
    datasets: [
      {
        ...
        data: Object.keys(graphicData).length
          ? graphicData[Number(month) + 1][type]
          : [],
        backgroundColor:
          type === "credit"
            ? [
  ...
  return (
    <div className={style.container}>
      {type === "credit" && totalCredit === 0 ? (
        <div>No Credit transactions made this month</div>
      ) : type === "debit" && totalDebit === 0 ? (
        <div>No Debit transactions made this month</div>
      ) : Object.keys(graphicData).length ? (
        <Doughnut data={data} />
      ) : (
        <div>Loading...</div>
      )}
