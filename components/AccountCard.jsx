...
function AccountCard({ acc }) {
      ...
      if (result.isConfirmed) {
        try {
          const { data: res } = await axios.delete(
            `https://myfin-sergioomarsanchez.vercel.app/api/transactions/?accountId=${acc._id}`
          );
          const { data: deleted } = await axios.delete(
            `https://myfin-sergioomarsanchez.vercel.app/api/accounts/${acc._id}`
          );
  ...
  return (
    <div key={acc._id} className={style.container}>
      <Image
        className={style.entityLogo}
        alt=""
        src={acc.logo}
        width={25}
        height={25}
      />
      <h3 className={style.entityName}>
        {acc.entityName} <span className={style.type}>{acc.accountType}</span>
        account
      </h3>
      <h3 className={style.entityName}>
      ...
      <div className={style.actions}>
        <span onClick={handleDelete} className={style.deleteAcc}>
          - Delete Account
        </span>
        <span className={style.addTransaction} onClick={() => setIsOpen(true)}>
          + Add transaction
        </span>
        {isOpen ? (
          <AddTransactionForm
        ...
        <div className={style.transactionsContainer}>
          {loading ? (
            ...
            <div className={style.noTransactions}>
              No Transactions added. <br></br>-{" "}
              <span
                className={style.addTransaction}
