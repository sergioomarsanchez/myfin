...
export default function reducer(state = initialState, action) {
  switch (action.type) {
        ...
        return {
          ...
          transactions: {
            ...state.transactions.filter(
              (t) => Object.keys(t)[0] === action.accountId
            ),
    ...
    case DELETE_TRANSACTION:
      const transactionFiltered = state.transactions[action.accountId].filter(
    ...
    case DELETE_ACCOUNT:
      const filteredAccounts = Object.keys(state.transactions)
    ...
    case UPDATE_ACC_BALANCE:
      const accToUpdate = allAccounts.find((account) => {
        account._id === action.payload.id;
      });
      return {
        ...state,
        allAccounts: [
          ...state.allAccounts.filter((account) => {
            account._id !== action.payload.id;
          }),
          { ...accToUpdate, balance: action.payload.newBalance },
        ],
      };
    case UPDATE_TOTALS:
        ...
        return {
          ...
          totalARS:
            action.payload.transactionType === "credit"
              ? state.totalARS + Number(action.payload.amount)
              : state.totalARS - Number(action.payload.amount),
        };
      } else {
        return {
          ...
          totalUSD:
            action.payload.transactionType === "credit"
              ? state.totalUSD + Number(action.payload.amount)
              : state.totalUSD - Number(action.payload.amount),
        };
      }
