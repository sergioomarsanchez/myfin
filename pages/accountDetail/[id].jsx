...
import {
  getMonthlyCreditDebit,
  getMonthlyGraphicData,
} from "../../graphicFunctions/utils";
...
function AccountDetail({ acc }) {
  ...
  useEffect(() => {
    if (transactions[acc._id]) {
      setAccTransactions(
        transactions[acc._id]?.sort(function (a, b) {
  ...
  const data = {
    ...
    datasets: [
      {
        label: "Debit",
        data: graphicData?.find((obj) => Object.keys(obj)[0] === year)[year][
          "debit"
        ],
          ...
          <div className={style.header}>
            <div className={style.account}>
              ...
              <h2 className={style.title}>
                {account.entityName} {account.type}
              </h2>
            </div>
            <div className={style.balanceYear}>
              <h4 className={style.totals}>
                Balance is:{" "}
                <span
                  style={{ color: account.balance > 0 ? "#4ada84" : "red" }}
                  className={style.totalammount}
                >
              ...
              <div className={style.yearSelectorContainer}>
                ...
                >
                  {graphicData?.map((year) => {
                    return <option key={year}>{Object.keys(year)[0]}</option>;
                  })}
                </select>
              </div>
            </div>
          ...
          <div className={style.graphicContainer}>
            {yearTransactions.length ? (
              <div className={style.yearGraphicContainer}>
                <div className={style.formatSelectorContainer}>
                  ...
                  <select
                    className={style.formatSelector}
                    name="format"
                    value={graphicFormat}
                    onChange={(e) => setGraphicFormat(e.target.value)}
                    id=""
                  >
              ...
              <div className={style.message}>
                Sorry No Transactions found in the year slected, try other
              </div>
            )}
            <div className={style.monthGraphicsContainer}>
              ...
              {yearTransactions.length ? (
                ...
                <div className={style.message}>
                  No Transactions found in the year slected
                </div>
              )}
            </div>
          </div>
...
export const getServerSideProps = async ({ params }) => {
  const acc = await axios.get(
    `https://myfin-sergioomarsanchez.vercel.app/api/accounts/` + params.id
  );
  return {
    props: {
      acc: acc.data,
