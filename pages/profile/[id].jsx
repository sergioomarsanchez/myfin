...
function Profile({user}) {
  ...
  useEffect(() => {
    if(acc.length) {
      Promise.all(acc.map(acc => fetchTransactionsForAccount(acc._id)))
        .then(() => {
      ...
      { id && token ?
        <div className={style.wrapper}>
          ...
          <div className={style.division}>
            <h3>Your accounts</h3>
            <div className={style.line}/> <div className={style.addAccount} onClick={()=>setIsOpen(true)}>+ Add New Account</div>
          </div>
        ...
        <div className={style.warning}>Sorry you have no credentials, please, Log in again</div>
      }
    </div>
  )
}
